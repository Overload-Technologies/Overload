	...
	sceneManager(projectAssetsPath.string())
{
	...
	if (!windowIconPath.empty())
	{
		...
		{
			// Ideally we would check if `SetIcon` succeeds so in case the windowIconRealPath didn't resolve
			// to valid texture data, we could still use the fallback window icon.
			window->SetIcon(windowIconRealPath.string());
