...
namespace
{
	std::filesystem::path ResolveApplicationIconPath(
		std::string p_iconSetting,
		const std::filesystem::path& p_projectAssetsPath,
		const std::filesystem::path& p_engineAssetsPath
	)
	{
		OvTools::Utils::String::Trim(p_iconSetting);

		if (p_iconSetting.empty())
		{
			return {};
		}
	...
	sceneManager(projectAssetsPath.string())
{
...

	{
		...
		{
			if (std::filesystem::exists(applicationIconPath))
			{
				window->SetIcon(applicationIconPath.string());
			}
			else
			{
				OVLOG_WARNING("Application icon file not found: " + applicationIconPath.string());
			}
		}
	}
