	...
	sceneManager(projectAssetsPath.string())
{
	...
	if (!windowIconPath.empty())
	{
		...
		{
			if (window->SetIcon(windowIconRealPath.string()))
			{
				useFallbackWindowIcon = false;
			}
			else
			{
				OVLOG_WARNING("Failed to load application icon file as texture: " + windowIconRealPath.string());
			}
		}
		else
		{
			OVLOG_WARNING("Application icon file not found: " + windowIconRealPath.string());
		}
	}
